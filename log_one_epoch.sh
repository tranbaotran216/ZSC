...
echo "==================================================="
echo "LOG TRAIN - VAL - TEST ONE EPOCH (U-Net + GroundingDINO) model arch version ${MODEL_ARCH_VER}"
...
python -u "$PROJECT_DIR/train.py" \
    --log_one_epoch \
    --resume D:/projects/zsc/checkpoints_dgdcount/model_arch_ver_2/unet_gdino_text_only_arch_ver_2/last_epoch_21.pth \
    --data-root "$DATA_ROOT" \
    --ann "$ANN_FILE" \
    --img-root "$IMG_ROOT" \
    ...
    2>&1 | tee "$SAVE_DIR/${EXP_NAME}epoch_21.log"
