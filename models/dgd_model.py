...
class DiffusionGroundingDINOCounter(nn.Module):
    ...
    def set_frozen_parameters(self):
        ...
        if hasattr(self.gd_model, 'bert'):
            for param in self.gd_model.bert.parameters():
                param.requires_grad = False
        if hasattr(self.gd_model, 'input_proj'):
            for param in self.gd_model.input_proj.parameters():
                param.requires_grad = False
        if hasattr(self.gd_model, 'backbone'):
            for param in self.gd_model.backbone.parameters():
                param.requires_grad = False
        
    def extract_unet_features(self, img: torch.Tensor, prompts: List[str]) -> Tuple[Tuple, torch.Tensor]:
        with torch.no_grad():
            latents = self.vae.encode(img)
            # Lưu ý: nếu dùng LDM gốc, đôi khi phải gọi self.clip.get_learned_conditioning(prompts)
            # Tạm giữ .encode() theo code của bạn nếu wrapper custom đã hỗ trợ
        ...
        # return: outs: (img_feat_list, attn12, attn24, attn48); c_crossattn: text_feat
    ...
    def forward(self, x: torch.Tensor, prompts: List[str], targets: Any = None, exemplars: Any = None, labels: Any = None, **kwargs):
        ...
        for feat in proj_img_feats:
            ...
            srcs.append(feat)
            masks.append(mask)
            pos_embed = self.gd_model.backbone[1](NestedTensor(feat, mask)).to(feat.dtype)
            pos_embeds.append(pos_embed)

        tokenized = self.gd_model.tokenizer(prompts, padding="longest", return_tensors="pt").to(x.device)
        attention_mask = tokenized.attention_mask.bool()
        # Extract BERT features
        text_feat = self.gd_model.bert(tokenized.input_ids, attention_mask=tokenized.attention_mask)[0]
        # text_feat.shape: torch.size([batch_size, len, 768])
        text_features = self.gd_model.feat_map(text_feat)

        # TẠO SQUARE SELF-ATTENTION MASK: shape (B, L, L)
        # Entry (i, j) = True chỉ khi cả token i và token j đều không phải là padding
        ...
        text_dict = {
            ...
            "position_ids": torch.arange(text_features.shape[1], device=x.device).unsqueeze(0).repeat(x.shape[0], 1),
